...
const (
	...
	controllerName = "multus-endpointslice-controller.npwg.k8s.io"
	...
	// multusServiceNetworkAnnotation
	multusServiceNetworkAnnotation = "k8s.v1.cni.cncf.io/service-network"
)
...
func NewController(podInformer coreinformers.PodInformer,
	...
	recorder := broadcaster.NewRecorder(scheme.Scheme, v1.EventSource{Component: "multus-endpoint-slice-controller"})

	if client != nil && client.CoreV1().RESTClient().GetRateLimiter() != nil {
		ratelimiter.RegisterMetricAndTrackRateLimiterUsage("multus_endpoint_slice_controller", client.DiscoveryV1().RESTClient().GetRateLimiter())
	}
...
func (c *Controller) syncService(key string) error {
	...
	esLabelSelector := labels.Set(map[string]string{
		...
		discovery.LabelServiceName: fmt.Sprintf("%s", service.Name),
