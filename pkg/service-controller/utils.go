...
func getEndpointAddresses(pod *corev1.Pod, service *corev1.Service, addressType discovery.AddressType) []string {
	...
	if strings.Index(serviceNetwork, "/") == -1 {
		serviceNetwork = fmt.Sprintf("%s/%s", pod.Namespace, strings.ReplaceAll(serviceNetwork, " ", ""))
	}

	for _, s := range statuses {
		if s.Name == serviceNetwork {
			addresses = append(addresses, s.IPs...)
		}
	}
...
func setEndpointSliceLabels(epSlice *discovery.EndpointSlice, service *corev1.Service) (map[string]string, bool) {
	...
	svcLabels[discovery.LabelServiceName] = fmt.Sprintf("%s", service.Name)
	svcLabels[discovery.LabelManagedBy] = controllerName
